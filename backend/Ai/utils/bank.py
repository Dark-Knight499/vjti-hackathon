...
load_dotenv()
#write a function to conduct a transaction
def transfer(account_number: str, transfer_account_number: str, amount: float) -> dict:
    """Conducts a transaction between two accounts.
    Args:
        account_number: The account number to transfer from
        transfer_account_number: The account number to transfer to
        amount: The amount to transfer
    Returns:
        Dictionary with the status of the transaction or error message
    """
    try:
        response = requests.get(f"http://localhost:5000/api/transfer/{account_number}/{transfer_account_number}/{amount}")
        data = response.json()
        return data
